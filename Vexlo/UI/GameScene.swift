...
final class GameScene: SKScene {
    ...
    private func applyEmptyCellStyle(_ node: SKShapeNode) {
        let isOpeningState = engine.scoreEngine.score == 0 && !engine.didClearAny
        node.fillColor = UIColor(white: 1, alpha: isOpeningState ? 0.064 : 0.055)
        node.strokeColor = UIColor(white: 1, alpha: isOpeningState ? 0.15 : 0.13)
        node.lineWidth = 0.95
        node.alpha = 1.0
        node.setScale(1.0)
        ...
        material.fillColor = UIColor(hex: engine.isDailyChallenge ? "F8FBFF" : "A8B4FF").withAlphaComponent(
            isOpeningState ? (engine.isDailyChallenge ? 0.03 : 0.023) : (engine.isDailyChallenge ? 0.021 : 0.016)
        )
    ...
    private func applyFilledCellStyle(_ node: SKShapeNode, color: UIColor) {
        ...
        if let highlight = node.childNode(withName: "fillHighlight") as? SKShapeNode {
            ...
            highlight.fillColor = UIColor(white: 1, alpha: 0.11)
            highlight.strokeColor = UIColor(white: 1, alpha: 0.18)
